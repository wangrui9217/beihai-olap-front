...
export default {
  ...
  methods: {
    ...
    arrivedSuccess() {
      ...
      this.loading = true
      api.arriveArbitralDecisionAwardSuccess(params).then(res => {
        this.loading = false;
        if (res.data.code === '1') {
    ...
    formatEmailData(data){
      ...
      data.forEach(item => {
        ...
        if(item.isUpload === 1 && !item.documentGeneratorUrlDes){
          item.updateTime = '暂无'
          item.disabled = true
        }
        item.unSend = true
    ...
    getDecisionGiveList() {
      ...
      this.loading = true
      api.getArbitralDecisionAwardArrive(params).then(res => {
        this.loading = false
        if (res.data.code === '1') {
          ...
          tableData.forEach(item => {
            ...
            item.onlineSome = this.setDocumentStatus({doucumentSum: item.doucmentCount, documenSuccessSum: item.successCount})
            item.expressSome = this.setDocumentStatus({doucumentSum: item.expressCount, documenSuccessSum: item.expressArrivedCount})
          })
    ...
    emailSubmit(){
      ...
      api.descisionDocumentBatch({
        "id": this.emailData[0].arbitralInfoId,
        "arbitralDocumentIds": ids,
        "arbitralInfoIds": arbitralInfoIds
      }).then(res => {
        this.loading = false
    ...
    upload(data, callback){
      this.loading = true
      api.uploadArbitralDocuments(data).then(res => {
        this.loading = false
        if (res.data.code === '1') {
          this.$message.success(res.data.msg)
          callback()
