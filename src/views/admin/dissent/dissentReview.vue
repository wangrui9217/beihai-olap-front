      ...
      <el-col>
        <el-table :data="tableData" ref="table" border style="width: 100%" >
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <span class="g-link" @click="handelCheck(scope.row)">核对</span>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
            ...
            if(item.documentType === 8){   // 调解书
              documentData.push({
                ...
                attachmentDatas: [{
                  ...
                  attachProperty: '',
                  attachStatus: 1,
                  url: api.getPdfURL(item.restartUpaloadUrl)
                }]
              })
            }
            else{    // 调解笔录
    ...
    getList(){
      ...
      this.loading = true
      api.getArbitralDecisionCheckInfos(params).then(res => {
        this.loading = false
        if(res.data.code === '1'){
    ...
    download(row){
      ...
      api.getMediationDocument({
        ...
        if(res.data.code === '1'){
          ...
          data.forEach(item => {
            let url = api.getPdfURL(item.restartUpaloadUrl)
