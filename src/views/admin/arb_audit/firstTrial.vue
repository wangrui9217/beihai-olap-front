<template>
  <div class="g-listpage__style" v-loading.fullscreen="loading">
    ...
    <el-row class="mt20">
      <el-col>
        <el-table :data="tableData" ref="table" border style="width: 100%" @selection-change="handleSelectionChange" row-key="id">
          ...
          <el-table-column label="附件" :show-overflow-tooltip="true" width="180">
            <template slot-scope="scope">
              <span class="revoke" @click="downloadFile(scope.row)">{{scope.row.recallPetitionName}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="recallReason" :show-overflow-tooltip="true" label="备注">
          </el-table-column>
        </el-table>
    ...
    <!--弹窗-->
    <review-dialog ref="reviewDialog" title="退回审核初审" :reviewFn="review" ></review-dialog>
  </div>
</template>
<script>
import { api } from '@/api'
...
export default {
  ...
  methods: {
    downloadFile (row) {
      let url = api.getPdfURL(row.recallPetitionUrl)
      DownloadFile(url, row.recallPetitionName)
      // downloadFileTool()
    },
    into(row) {
    ...
    getBranchRecallInfos(){
      ...
      this.loading = true
      api.getBranchRecallInfos(params).then(res => {
        this.loading = false
        if(res.data.code === '1'){
      ...
      if (passOrNot == '2' && reviewTextarea == '') {
        ...
        let requests = this.selectedArbitral.map(item => {
            return {
              ...
              "id": item.id,
              "recallApplication": reviewTextarea,
              "status": status
            }
          })
        let params = {
        ...
        this.loading = true;
        api.addRecallInfoFirstReview(params).then((res) => {
          this.loading = false;
