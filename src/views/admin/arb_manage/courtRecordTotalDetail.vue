<template>
  <div class="g-proofread" v-loading.fullscreen="loading">
    <div class="ly-header">
      <div class="msg">
        <h2>{{arbiInfo.mergeHearNumber}}</h2>
        <div class="box">
          <p style="width: 100%;"><span>案件编号：</span>
            <span class="ellipsis" :title="arbiInfo.arbitralEvidenceResponse.map(val => val.arbNumber).join()">{{arbiInfo.arbitralEvidenceResponse.map(val => val.arbNumber).join()}}</span>
            </p>
        </div>
        <div class="box">
          <p><span>申请人：</span>
            <span class="ellipsis" :title="arbiInfo.prosecutors">{{arbiInfo.prosecutors}}</span>
            </p>
          <p><span>仲裁员：</span>
            <span class="ellipsis" :title="arbiInfo.arbitrators">{{arbiInfo.arbitrators}}</span></p>
          <p><span>庭审地点：</span>
            <span class="ellipsis" :title="arbiInfo.trialLocation">{{arbiInfo.trialLocation}}</span>
            </p>
        </div>
        <div class="box">
          <p><span>被申请人：</span>
            <span class="ellipsis" :title="arbiInfo.defendants">{{arbiInfo.defendants}}</span></p>
          <p><span>仲裁庭组成：</span>
            <span v-if="arbiInfo.arbTrialComposition === 1">独任</span>
            <span v-if="arbiInfo.arbTrialComposition === 2">合议</span>
          </p>
          <p><span>庭审时间：</span>
            <span>{{arbiInfo.trialBeginTime}}-{{arbiInfo.trialEndTime}}</span>
          </p>
        </div>
      </div>
      <div class="actions">
    ...
    <show-evidence :arbi-info="arbiInfo" :hasReply="true">
      ...
      <div class="select">
        <el-upload
          ...
          <el-button size="small" type="default" icon="el-icon-upload2">{{attachmentDatas.length === 0 ? '上传':'重新上传'}}</el-button>
        </el-upload>
...
export default {
  ...
  computed: {
    ...
    uploadUrl () {
      return `${api.uploadMergeHearRecord}?arbitralMergeInfoId=${this.arbiInfo.arbitralMergeInfoId}`
    }
  },
  methods: {
    download(row){
    ...
    queryData(val){
      this.loading = true;
      api.queryArbitralMergeCourtRecord({
        id: this.id
      }).then((res) => {
        this.loading = false;
        if(res.data.code == '1'){
    ...
    confirmPass() {
      ...
      this.loading = true
      api.hearMergeJudgeOUploadRecord({
        "tribunalStatus": 3,
        "htmlstr": courtRecord,
        "arbitralInfoIds": this.arbiInfo.arbitralEvidenceResponse.map(val => val.arbitralInfoId), //案件id
        "arbitralMergeInfoId": this.arbiInfo.arbitralMergeInfoId //案件流程id
      }).then((res) => {
        this.loading = false;
        if (res.data.code == '1') {
    ...
    save(){
      ...
      courtRecord = this.htmlString.replace(/<div class=\"wrap\">([\s\S]*?)<\/body>/, `<div class="wrap">${courtRecord}</div></body>`)
      this.loading = true
      api.saveMergeHearRecordContext({
        mergeHearRecordContext: courtRecord,
        arbitralMergeId: this.arbiInfo.arbitralMergeInfoId
      }).then((res) => {
        this.loading = false;
        if (res.data.code == '1') {
          this.$message.success(res.data.msg);
    ...
    queryHearRecordContext(){
      api.queryMergeHearRecordContext({
        id: this.id
      }).then((res) => {
        if(res.data.code == '1'){
          this.htmlString = res.data.data
  ...
  created(){
    ...
    if(this.id){
      // 1,简易；2，简易（缺席审理）
      this.queryHearRecordContext()
      api.getArbitralInfoDetailForMergeHearRecord({
        id: this.id
      }).then((res) => {
        if(res.data.code == '1'){
          this.arbiInfo = {...this.arbiInfo, ...res.data.data}
