...
export default {
  ...
  methods: {
    ...
    arrivedSuccess() {
      ...
      let httpFn = null
      if(this.arbStatus == 41){
      // 线下开庭
        httpFn = api.hearJudgePaperReach
      }
      else if(this.arbStatus == 44){
        // 书面审理
        httpFn = api.hearJudgeUploadPaperReach
      }
      this.loading = true
      httpFn({processCommonRequests: params}).then(res => {
        this.loading = false;
        if (res.data.code === '1') {
    ...
    getArbitralInfosBySys() {
      ...
      params = Object.assign(this.formInline, {
        ...
        arbStatus: this.arbStatus,
        arbitralTribunalState: 2
      })
      let httpFn = null
      if(params.arbStatus === 41){
      // 线下开庭
        httpFn = api.getOfflineTribunalArriveArbitralInfos
      }
      else if(params.arbStatus === 44){
        // 书面审理
        httpFn = api.getAdjudicationByRecordArriveArbitralInfos
      }
      this.loading = true
      httpFn(params).then(res => {
        this.loading = false
        if (res.data.code === '1') {
