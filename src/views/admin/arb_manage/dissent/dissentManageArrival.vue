    ...
    <div class="g-listpage__style g-send-document">
      ...
      <div class="mt20">
        <el-table :data="tableData" border style="width: 100%" @selection-change="selectionChange" ref="table" row-key="id">
          ...
          <el-table-column prop="decisionProsecutorName" :show-overflow-tooltip="true" label="异议申请人">
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    arrivedSuccess() {
      ...
      this.loading = true
      api.decisionDocumentArrivedSuccess(params).then(res => {
        this.loading = false;
        if (res.data.code === '1') {
    ...
    getDecisionGiveList() {
      ...
      this.loading = true
      api.getDecisionGiveList(params).then(res => {
        this.loading = false
        if (res.data.code === '1') {
