      ...
      <div class="ly-right">
        ...
        <div class="select">
          ...
          <el-select v-model="evidenceName" @change="getEvidence" style="height: 30px;">
            <el-option v-for="item in evidences" :value="item.attachmentEvidenceId" :label="item.evidenceName" :key="item.attachmentEvidenceId"></el-option>
          </el-select>
        </div>
        <div class="select1">
          <p>证据文件：</p>
...
export default {
  ...
  watch: {
    arbiInfo: function() {
      this.arbitralEvidenceResponse = this.arbiInfo.arbitralEvidenceResponse
      this.caseNumber = this.arbitralEvidenceResponse[0].arbitralInfoId
      this.evidences = this.arbitralEvidenceResponse[0].envidence
      this.evidenceName = this.evidences[0].attachmentEvidenceId
      this.attachments = this.evidences[0].arbAttachmentDatas
  ...
  methods: {
    ...
    getEvidence(val) {
      let temp = this.evidences.find(value => value.attachmentEvidenceId === val) || {}
      this.attachments = temp.arbAttachmentDatas || []
      this.attachUrlDes = ''
    },
    getAttachment(val) {
      if (val) {
        ...
        let endName = this.attachment.attachName.substring(this.attachment.attachName.lastIndexOf('.')).toLowerCase()
